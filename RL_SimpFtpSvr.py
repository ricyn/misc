    ...
    def handle_LIST(self, arg):
        ...
        for filename in os.listdir(local):
            ...
            if os.path.isfile(path) or os.path.isdir(path): # ignores link or block file
                status = os.stat(path)
                if filename==__file__:continue # 本语句对客户端隐藏服务器脚本,如果脚本文件名复杂可以减小被攻击的几率
                msg = template % (
...
def usage():
    ...
    -h help
    -p listen port
    -o output log to a log file, by default it outputs to stdout
...
def param_handler(opts):
    ...
    be_daemon = False
    # logger = get_logger(logging.FileHandler(logfile)) # log默认输出到文件
    logger = get_logger() # log默认输出到stdout
    for o, a in opts:
        if o == '-h':
            usage()
            sys.exit(0)
...
"""
    try:
        ...
        opts, args = getopt.getopt(['-t'], 'hdp:ot') # Windows下不支持fork模式,强行修改命令行
    ...
    lan_ip=socket.gethostbyname(socket.getfqdn(socket.gethostname()))
    logger.info("shared directory: %s" % (default_home_dir))
    logger.info("accessible address: ftp://%s:%s" % (lan_ip,port))

    # socket.setdefaulttimeout(timeout) # 非阻塞模式,超时
    # socket.setdefaulttimeout(None) # 阻塞模式
